...
export function Hallmark({ initials, metal = 'copper', className }: { initials: string; metal?: Metal; className?: string }) {
  ...
  useTilt(hold, 10);
  return (
    <span ref={hold} className={['cp-medal', className].filter(Boolean).join(' ')} style={{ aspectRatio: '120 / 64' }}>
    <Relief width={120} height={64} metal={metal} depth={3.2} soft={0.6} sweep label={`Maker's mark, ${initials}`} style={{ display: 'block', width: '100%', height: '100%' }}>
